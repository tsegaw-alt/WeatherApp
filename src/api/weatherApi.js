...
export const fetchWeather = async (lat, lon) => {
  try {
    return await api.get(URLS.fetchWeathersUrl, {
      baseURL: `${weatherApi}lat=${lat}&lon=${lon}&exclude=hourly,minutely${appId}${unitMetric}`,
    });
  } catch (error) {
    console.error(error);
  }
  ...
  try {
    const responses = await Promise.all(
      historyDates().map(async (date) => {
        return await api.get(URLS.fetchWeathersUrl, {
          baseURL: `${historyWeatherApi}lat=${lat}&lon=${lon}&dt=${date}${appId}&only_current={true}${unitMetric}`,
        });
      })
    );
    return responses;
...
export const fetchGeoCode = async (name) => {
  try {
    const coordinate = [];
    const geoCodingResult = await api.get(URLS.fetchWeathersUrl, {
      baseURL: `${fetchGeoCodeApi} ${name}  ${appId}`,
    });
